...
window.onload = function init()
{
    ...
    var T = Mat.translation(new PV(-0.1, 0.1, -1.5, false));
    var R = Mat.rotation(1, 0.3);
    var S = Mat.scale(0.3);
        
    cube2world = T.times(R).times(S);
    ...
    function lookAtL () {
        ...
        // light position in world coordinates is lightP
        // object (center) position in object coordinates is (0,0,0) 
        // proj2clipL is the identity.
        // Use the existing view2proj.

        var proj2clipL = new Mat();
        var view2clipL = proj2clipL.times(view2proj);

        var object = rotated2world.times(object2rotated).times(new PV(true));
        var wy = new PV(0, 1, 0, false);
        var vz = lightP.minus(object).unit();
        var vx = wy.cross(vz).unit();
        var vy = vz.cross(vx);
        var R = new Mat(vx, vy, vz);
        var Rinv = R.transpose();
        var T = Mat.translation(lightP);
        var Tinv = Mat.translation(lightP.minus());

        var world2viewL = Rinv.times(Tinv);

        world2clipL = view2clipL.times(world2viewL);
    }
...
function render() {
	...
	gl.clear(gl.DEPTH_BUFFER_BIT);
        ...
        // EXERCISE 3
        // Set model2world in the vertex shader to model2world in view.js.
        // Set world2clip in the vertex shader to world2clipL.
        // You should see the sphere from the light's point of view.
        // It will be "fat" because proj2clipL is the identity.
        
        gl.uniformMatrix4fv(gl.getUniformLocation(program, "model2world"), false, model2world.flatten());
        gl.uniformMatrix4fv(gl.getUniformLocation(program, "world2clip"), false, world2clipL.flatten());

        sphere.render(gl, program, flatOrRound); // Render the sphere.
        
        // EXERCISE 5
        // Set model2world in the shader to cube2world.
        // Render the cube.
        // You should see the cube blocking the lower right of the sphere.

        gl.uniformMatrix4fv(gl.getUniformLocation(program, "model2world"), false, cube2world.flatten());
	...
	depthTexture.bind(gl);
        ...
        // Send world2clipL to the vertex shader.

        gl.uniformMatrix4fv(gl.getUniformLocation(program, "world2clipL"), false, world2clipL.flatten());
        ...
        gl.uniformMatrix4fv(gl.getUniformLocation(program, "model2world"), false, model2world.flatten());
        gl.uniformMatrix4fv(gl.getUniformLocation(program, "world2clip"), false, world2clip.flatten());

        sphere.render(gl, program, flatOrRound);
        
        // EXERCISE 8
        // Set model2world in shader to cube2world.
        // Render the cube (uncomment the render below).
        // The cube should block the sphere, but towards the upper left.

        gl.uniformMatrix4fv(gl.getUniformLocation(program, "model2world"), false, cube2world.flatten());

        cube.render(gl, program, 1);
