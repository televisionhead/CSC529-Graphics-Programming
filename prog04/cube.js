class Cube {
    constructor (gl) {
        ...
        // See square.js for the incantation.
         ...
         for(var i = 0; i < this.faces.length; i++) {
            this.element_buffers.push(gl.createBuffer());
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.element_buffers[i]);
            gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, flattenElements(this.faces[i]), gl.STATIC_DRAW);
         }
    ...
    render (gl, program) {
        ...
        for(var i = 0; i < this.faces.length; i++) {
            gl.uniform4fv(color, this.colors[i]);
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.element_buffers[i]);
