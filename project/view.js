...
window.onload = function init()
{
    ...
    */
    ...
    var moves = [];
    var frameTime = 1000.0 / 60.0; //60 FPS = 60 frames per 1000 ms = about 16.666 ms per frame

    document.getElementById("play").onclick = function () {
        var count = 0;

        var intervalId = window.setInterval(function() {
            moves[count][0].part2joint = moves[count][1];
            moves[count][0].joint2part = moves[count][2];
            updateM2C();
            count++;
        ...
        }, moves.length / frameTime); //we have moves.length total frames, each one should get frameTime ms
    }
    ...
    document.getElementById("recordstart").onclick = function () {
        ...
        var intervalId = window.setInterval(function() {
            if(closestHit != null) {
                moves.push([closestHit, closestHit.part2joint, closestHit.joint2part]);

                if(!IDs.includes(closestHit.id)) {
                    IDs.push(closestHit.id);
                    if(closestHit.part2joint_old != null && closestHit.joint2part_old != null) {
                        restore.push([closestHit, closestHit.part2joint_old, closestHit.joint2part_old]);                        
                    }
                }
            }
            ...
            if(!recording) {
                for(var i = 0; i < restore.length; i++) {
                    restore[i][0].part2joint = restore[i][1];
                    restore[i][0].joint2part = restore[i][2];
                    updateM2C();
                }
                clearInterval(intervalId);
            } 
        }, frameTime); //we record 1 frame every 16.666 ms
    }

    document.getElementById("recordstop").onclick = function () { recording = false; }
