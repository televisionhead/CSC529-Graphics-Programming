...
class Part {
    constructor (sphere, flatOrRound,
        ...
        this.part2joint_old = null;
        this.joint2part_old = null;
    }
    ...
    drag (front, back) {
        ...
        if(this.part2joint_old == null) {
            this.part2joint_old = this.part2joint;
            this.joint2part_old = this.joint2part;
        }
...
function makeTorso(lon, lat, scaleX, scaleY, scaleZ) {
    var sphere = new Sphere(gl, lon, lat);
    var sphere2part = Mat.scale(new PV(scaleX, scaleY, scaleZ, false));
    var part2sphere = Mat.scale(new PV(1.0 / scaleX, 1.0 / scaleY, 1.0 / scaleZ, false));
    return new Part(sphere, 1, sphere2part, part2sphere, new Mat(), new Mat());
}

function makeBodyPart(parent, t, t2, lon, lat, scaleX, scaleY, scaleZ, shiny) {
    var joint2parent = Mat.translation(t);
    var parent2joint = Mat.translation(t.minus());
    var sphere = new Sphere(gl, lon, lat);
    var sphere2part = Mat.scale(new PV(scaleX, scaleY, scaleZ, false)).times(Mat.translation(t2));
    var part2sphere = Mat.translation(t2.minus()).times(Mat.scale(new PV(1.0 / scaleX, 1.0 / scaleY, 1.0 / scaleZ, false)));
    var part = new Part(sphere, shiny, sphere2part, part2sphere, joint2parent, parent2joint);
    parent.children.push(part);
    return part;
}

function makeArmOrLeg(body, x, y) {
    var leftbase = makeBodyPart(body, new PV(-x, y, 0, false), new PV(-0.707, 1, 0, false), 6, 3, 0.5, 0.2, 0.2, 1);
    var rightbase = makeBodyPart(body, new PV(x, y, 0, false), new PV(0.707, 1, 0, false), 6, 3, 0.5, 0.2, 0.2, 1);
    var leftjoint = makeBodyPart(leftbase, new PV(-0.8, 0.2, 0, false), new PV(-0.8, 0, 0, false), 6, 3, 0.3, 0.2, 0.2, 1);
    var rightjoint = makeBodyPart(rightbase, new PV(0.8, 0.2, 0, false), new PV(0.8, 0, 0, false), 6, 3, 0.3, 0.2, 0.2, 1);
    var leftforebase = makeBodyPart(leftjoint, new PV(-0.5, -0.2, 0, false), new PV(-0.5, 1, 0, false), 6, 3, 0.5, 0.2, 0.2, 1);
    var rightforebase = makeBodyPart(rightjoint, new PV(0.5, -0.2, 0, false), new PV(0.5, 1, 0, false), 6, 3, 0.5, 0.2, 0.2, 1);
    var leftend = makeBodyPart(leftforebase, new PV(-0.8, 0.2, 0, false), new PV(0, 0, 0, false), 6, 3, 0.2, 0.2, 0.2, 1);
    var rightend = makeBodyPart(rightforebase, new PV(0.8, 0.2, 0, false), new PV(0, 0, 0, false), 6, 3, 0.2, 0.2, 0.2, 1);
}
...
function makeBody (gl) {
    ...
    if (simple) {
        ...
        body = makeTorso(6, 4, 0.75, 1.3, 0.6);

        {
            var head = makeBodyPart(body, new PV(0, 1.7 * 0.707, 0, false), new PV(0, 1, 0, false), 6, 3, 0.5, 0.5, 0.5, 1);
            var lefteye = makeBodyPart(head, new PV(-0.2, 0.5, 0.3, false), new PV(0, 0, 0, false), 16, 16, 0.1, 0.1, 0.1, 2);
            var righteye = makeBodyPart(head, new PV(0.2, 0.5, 0.3, false), new PV(0, 0, 0, false), 16, 16, 0.1, 0.1, 0.1, 2);
        }

        makeArmOrLeg(body, 0.4, 0.4);
        makeArmOrLeg(body, 0.4, -0.8);
