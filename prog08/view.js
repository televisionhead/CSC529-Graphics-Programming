...
window.onload = function init()
{
    ...
    // Picture hangs on the back of the clip box.
    pictureBack = new Picture(gl, new PV(-1, 1, 0, true), new PV(1, 0, 0, false), new PV(0, -1, 0, false), 2, 2);

    // EXERCISE 3
    // Change pictureRight to hang on the right side of the clip box.
    // This will make the RightBack button start working.
    pictureRight = new Picture(gl, new PV(1, 1, -1, true), new PV(0, 0, 1, false), new PV(0, -1, 0, false), 2, 2);
    ...
    document.getElementById("Perspective").onclick = function () {
        console.log("You clicked perspective!");
    ...
    document.getElementById("RightBack").onclick = function () {
        right = !right;
    };

    document.getElementById("FlatSmooth").onclick = function () {
        console.log("You clicked FlatSmooth!");
        flatOrSmooth = flatOrSmooth != 1 ? 1 : 2;
    };

    document.getElementById("RefractReflect").onclick = function () {
        console.log("You clicked refract!");
        refract = !refract;
    };
...
function render() {
    ...
    var picture = right ? pictureRight : pictureBack;
    // EXERCISE 1B
    // Set picture2clip using picture.picture2world and world2clip (see updateMC2).
    var picture2clip = world2clip.times(picture.picture2world);

    // Send model2clip to shader program.
    var model2clipLoc = gl.getUniformLocation( program, "model2clip" );
    gl.uniformMatrix4fv(model2clipLoc, false, picture2clip.flatten());

    // Run shader program to render picture.
    picture.render(gl, program);

    gl.uniformMatrix4fv(model2clipLoc, false, model2clip.flatten());
    gl.uniform1i(gl.getUniformLocation(program, "refract"), refract);

    // EXERCISE 2A
    // Uncomment the render below.
    // Then set model2picture to the correct value.
    var model2picture = (picture.world2picture).times(model2world);

    // Send model2picture to shader program.
    var model2pictureLoc = gl.getUniformLocation( program, "model2picture" );
    gl.uniformMatrix4fv(model2pictureLoc, false, model2picture.flatten());

    // Run shader program to render sphere.

    sphere.render(gl, program, flatOrSmooth); // uncomment
